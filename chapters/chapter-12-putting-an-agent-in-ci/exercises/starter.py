"""
Chapter 12 Exercises: The CI Policy Layer's Building Blocks, Testable Offline

This file rebuilds the three small pieces `project/solution.py`'s
CI-specific policy layer is made of, as standalone functions: a
scratch-scope check (in_scratch), a protected-branch check
(is_protected_branch), and the fail-closed, mode-aware policy lookup
(resolve_tier) that stands in for `active_policy()` + `check_permission()`
combined. All three are testable completely offline -- no live model, no
Ollama, no network, no real git repo needed.

Fill in TODO 1 through TODO 3, then run:

    python3 starter.py

The built-in test harness at the bottom runs a set of checks and prints
PASS/FAIL for each. Compare against `solution.py` (`python3 solution.py`)
if you get stuck.
"""

ALLOWED = "ALLOWED"
REQUIRES_CONFIRMATION = "REQUIRES_CONFIRMATION"
BLOCKED = "BLOCKED"


def in_scratch(path, prefix):
    """
    Given a path (relative to a workspace root, e.g. "ci-scratch/summary.md"
    or "app.py") and a scratch prefix (e.g. "ci-scratch/"), return True if
    the path is inside that scratch prefix, False otherwise. This is the
    exact check `project/solution.py`'s `_in_ci_scratch()` performs.

    Handle both "ci-scratch/summary.md" (already normalized) and
    "./ci-scratch/summary.md" (a leading "./" some callers might pass) the
    same way -- both should return True for prefix "ci-scratch/".

    TODO 1: implement this. If `path` starts with the literal "./",
    strip exactly that two-character prefix (don't use str.lstrip() for
    this -- lstrip() strips ANY combination of the given characters
    repeatedly from the left, which would mangle a genuinely relative
    path like "../../etc/passwd" into "etc/passwd", silently losing the
    very thing that made it suspicious) before checking whether the
    result starts with `prefix`.
    """
    raise NotImplementedError("in_scratch: TODO 1")


def is_protected_branch(branch, protected_branches):
    """
    Given a branch name (or None, if it couldn't be determined) and a set
    of protected branch names, return True if the branch is protected OR
    if it's None (an undetermined branch should be treated as protected --
    "we don't know" is not a safe reason to allow a commit). Return False
    only if `branch` is a real, known, non-protected branch name.

    TODO 2: implement this in a few lines.
    """
    raise NotImplementedError("is_protected_branch: TODO 2")


def resolve_tier(name, ci_mode, policy, policy_ci):
    """
    The mode-aware, fail-closed policy lookup -- exactly what
    `active_policy()` + `check_permission()` do together in
    `project/solution.py`. Given a tool name, whether this run is in CI
    mode, and BOTH policy dicts, return the tool's tier:

    - If ci_mode is True, look `name` up in `policy_ci`.
    - If ci_mode is False, look `name` up in `policy`.
    - Either way, if `name` isn't a key in the chosen dict at all, return
      BLOCKED -- fail closed, regardless of which mode this is.

    TODO 3: implement this. Should be a few lines -- pick the right dict
    first, then do a fail-closed .get(...) lookup on it.
    """
    raise NotImplementedError("resolve_tier: TODO 3")


def main():
    checks = []

    def check(label, actual, expected):
        ok = actual == expected
        checks.append(ok)
        status = "PASS" if ok else "FAIL"
        print(f"[{status}] {label}")
        if not ok:
            print(f"       expected: {expected!r}")
            print(f"       actual:   {actual!r}")

    # --- in_scratch ---
    check("in_scratch: a file directly inside the prefix", in_scratch("ci-scratch/summary.md", "ci-scratch/"), True)
    check("in_scratch: a nested file inside the prefix", in_scratch("ci-scratch/sub/dir/file.txt", "ci-scratch/"), True)
    check("in_scratch: a leading ./ is normalized away", in_scratch("./ci-scratch/summary.md", "ci-scratch/"), True)
    check("in_scratch: a file outside the prefix", in_scratch("app.py", "ci-scratch/"), False)
    check(
        "in_scratch: a path that merely CONTAINS the prefix name isn't enough",
        in_scratch("not-ci-scratch/summary.md", "ci-scratch/"),
        False,
    )
    check("in_scratch: the bare prefix itself doesn't count as a file inside it", in_scratch("ci-scratch", "ci-scratch/"), False)
    check(
        "in_scratch: a genuine path escape is NOT accidentally normalized into scope (the lstrip() trap)",
        in_scratch("../../etc/passwd", "ci-scratch/"),
        False,
    )

    # --- is_protected_branch ---
    protected = {"main", "master"}
    check("is_protected_branch: main is protected", is_protected_branch("main", protected), True)
    check("is_protected_branch: master is protected", is_protected_branch("master", protected), True)
    check("is_protected_branch: a scratch branch is not protected", is_protected_branch("ci/auto-fix", protected), False)
    check("is_protected_branch: an undetermined branch (None) is treated as protected", is_protected_branch(None, protected), True)

    # --- resolve_tier ---
    policy = {"read_file": ALLOWED, "write_file": REQUIRES_CONFIRMATION, "edit_file": REQUIRES_CONFIRMATION}
    policy_ci = {"read_file": ALLOWED, "write_file": ALLOWED, "edit_file": BLOCKED}

    check("resolve_tier: interactive mode uses the interactive policy", resolve_tier("write_file", False, policy, policy_ci), REQUIRES_CONFIRMATION)
    check("resolve_tier: CI mode uses the CI policy for the SAME tool name", resolve_tier("write_file", True, policy, policy_ci), ALLOWED)
    check("resolve_tier: CI mode can be MORE restrictive for a tool too", resolve_tier("edit_file", True, policy, policy_ci), BLOCKED)
    check("resolve_tier: interactive mode for that same tool is less restrictive", resolve_tier("edit_file", False, policy, policy_ci), REQUIRES_CONFIRMATION)
    check(
        "resolve_tier: a tool unclassified in BOTH policies defaults to BLOCKED in interactive mode",
        resolve_tier("run_database_migration", False, policy, policy_ci),
        BLOCKED,
    )
    check(
        "resolve_tier: the SAME unclassified tool also defaults to BLOCKED in CI mode",
        resolve_tier("run_database_migration", True, policy, policy_ci),
        BLOCKED,
    )

    n_passed = sum(checks)
    print(f"\n{n_passed}/{len(checks)} checks passed.")


if __name__ == "__main__":
    main()
