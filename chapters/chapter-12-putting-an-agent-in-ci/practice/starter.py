...
actual TOOL_POLICY, TOOL_POLICY_CI, CI_SCRATCH_PREFIX, and
PROTECTED_BRANCHES -- not a hypothetical policy, the exact one this
chapter ships. For each one, predict the outcome under BOTH modes
(interactive and CI) before running the script -- several of these have
a twist that a plain tier lookup, in either mode alone, doesn't fully
capture.
